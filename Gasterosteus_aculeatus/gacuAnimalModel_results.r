...
pheno = read.table('gacuCrossovers.csv',header = TRUE,sep="\t",stringsAsFactor=F)
...
summary(gacu.heritability.both.C)
summary(gacu.heritability.female.C)
summary(gacu.heritability.male.C)
...
for(i in 1:3){
  print(i)
  if(i==1){
    tmp=gacu.heritability.both.C
    s="Both"
  } else if (i==2){
    tmp=gacu.heritability.female.C
    s="Female"
  } else if (i==3){
    tmp=gacu.heritability.male.C
    s="Male"
  }
  ...
  print(variables)
  #tmp.fixed=data.frame(Analysis="RepeatedObs",Sex=s,Type="Fixed",Estimate=variables)
  #tmp.fixed$mode=as.numeric(posterior.mode(tmp[["Sol"]][,variables]))
  
  
  if(s=="Both"){
    #tmp.fixed$median=as.numeric(apply(tmp[["Sol"]][,variables],2,median))
    ...
    #tmp.fixed$median=as.numeric(median(tmp[["Sol"]][,variables])) 
  ...
  #  R.repeated=c(R.repeated,R)
  #  h2.repeated=c(h2.repeated,h2)
...
plot.table.S2=subset(plot.table, Sex=="Both" & Estimate !="italic(\"V\")[italic(\"p\")]")
plot.table.S2=rbind(plot.table.S2, subset(plot.table, Sex=="Female" & Estimate %in% c("italic(R)","italic(\"h\")^italic(\"2\")","italic(\"V\")[italic(\"a\")]", "italic(\"V\")[italic(\"pe\")]","italic(\"V\")[italic(\"r\")]")))
plot.table.S2=rbind(plot.table.S2, subset(plot.table, Sex=="Male" & Estimate %in% c("italic(R)","italic(\"h\")^italic(\"2\")","italic(\"V\")[italic(\"a\")]", "italic(\"V\")[italic(\"pe\")]","italic(\"V\")[italic(\"r\")]")))



ggplot(plot.table.S2, aes(Value,after_stat(ndensity)))+ geom_freqpoly() + 
...
#ggsave(paste0("FigS2_gacuPosterior_",NITT,"-PriorC.pdf"),width = 120,height=80,units="mm")
#Va-Vpe correlation
plot.table2=data.frame(Sex=rep(c("Both","Female","Male"),each=2000),Va=va.repeated, Vpe=vpe.repeated, Species="G. aculeatus")
...
#write.table(plot.table2,file=paste0("FigS3_gacuVaVpe_",NITT,"-PriorC-Length.csv"),sep="\t",row.names=F,col.names=T,quote=F)
#ggsave(paste0("FigS3_gacuVaVpe_",NITT,"-PriorC.pdf"),width = 120,height=80,units="mm")


#ggplot(pheno.long,aes(x=reorder(animal,FUN=mean,CO,decreasing=F),y=CO,col=parentsex)) + geom_boxplot(outlier.size = 0.5,size=0.5)+
  scale_color_manual(name="Sex",values=COLORS) +theme_classic() + theme(axis.ticks.x = element_blank(),axis.text.x = element_blank(),legend.position =c(0.8,0.1),legend.direction = "horizontal")+
  xlab("Individual")+ ylab("Number of autosomal crossovers")+ggtitle(label=expression(italic("Gasterosteus aculeatus")))

#ggsave(filename = "gacu_boxplot.pdf",width=130,height=80,units="mm")


#Write result tables

#write.table(rbind(repeated.model.fixed.effects,repeated.model.random.effects),file=paste0(NITT,"-gacuHeritabilityEstimates-PriorC.csv"),sep="\t",row.names=F,col.names=T,quote=F)

rounded.out=rbind(repeated.model.fixed.effects,repeated.model.random.effects)
rounded.out[,5:9]=round(rounded.out[,5:9],4)
#write.table(rounded.out,file=paste0(NITT,"-gacuHeritabilityEstimatesRounded-PriorC.csv"),sep="\t",row.names=F,col.names=T,quote=F)
